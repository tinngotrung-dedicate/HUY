...
const ensureDb = () => {
  if (!db) {
    throw new ChatSDKError(
      ...
      "Database is not configured (POSTGRES_URL missing)"
    );
  }
  return db;
};
...
export async function createGuestUser() {
  ...
  try {
    return await dbInstance.insert(user).values({ email, password }).returning({
      id: user.id,
      email: user.email,
    });
  } catch (_error) {
    // Nếu lỗi DB (ví dụ không kết nối được), fallback sang guest tạm thời để tránh 500.
    const tempId = `local-guest-${Date.now()}`;
    return [{ id: tempId, email: "guest@local" }];
  }
}
...
export async function deleteChatById({ id }: { id: string }) {
  ...
  try {
    ...
    await dbInstance.delete(stream).where(eq(stream.chatId, id));

    const [chatsDeleted] = await dbInstance
      .delete(chat)
      .where(eq(chat.id, id))
      .returning();
    return chatsDeleted;
...
export async function deleteAllChatsByUserId({ userId }: { userId: string }) {
  const dbInstance = ensureDb();
  try {
    const userChats = await dbInstance
      .select({ id: chat.id })
      .from(chat)
    ...
    await dbInstance.delete(vote).where(inArray(vote.chatId, chatIds));
    await dbInstance.delete(message).where(inArray(message.chatId, chatIds));
    await dbInstance.delete(stream).where(inArray(stream.chatId, chatIds));

    const deletedChats = await dbInstance
      .delete(chat)
      .where(eq(chat.userId, userId))
      .returning();
...
export async function deleteMessagesByChatIdAfterTimestamp({
  ...
  const dbInstance = ensureDb();
  try {
    const messagesToDelete = await dbInstance
...
export async function updateChatTitleById({
  ...
  const dbInstance = ensureDb();
  try {
    return await dbInstance.update(chat).set({ title }).where(eq(chat.id, chatId));
  } catch (error) {
...
export async function getStreamIdsByChatId({ chatId }: { chatId: string }) {
  const dbInstance = ensureDb();
  try {
    const streamIds = await dbInstance
      .select({ id: stream.id })
      .from(stream)
